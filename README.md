...
These functions are used in order to retrieve the relative distance and angle to a token that is in the input (`code`) which is a list containg codes of tokens.  
`find_token_new` is used for the first time in the `reach_dist_rot` function which allows the robot to reach the clustering point that corresponds to the code of the first seen token that is added to the input (`code`) list.  
